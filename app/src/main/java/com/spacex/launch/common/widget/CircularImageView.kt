package com.spacex.launch.common.widget
...
/**
 * Tranforms normal image to circular view.
 *
...
class CircularImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0)
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        circleColor = attributes.getColor(R.styleable.CircularImageView_circle_color, Color.WHITE)

        // Init Border
        if (attributes.getBoolean(R.styleable.CircularImageView_border, true)) {
            val defaultBorderSize = DEFAULT_BORDER_WIDTH * getContext().resources.displayMetrics.density
            borderWidth = attributes.getDimension(R.styleable.CircularImageView_border_width, defaultBorderSize)
            borderColor = attributes.getColor(R.styleable.CircularImageView_border_color, Color.WHITE)
        }

        // Init Shadow
        shadowEnable = attributes.getBoolean(R.styleable.CircularImageView_shadow, shadowEnable)
        if (shadowEnable) {
            shadowGravity = attributes.getInteger(R.styleable.CircularImageView_shadow_gravity, shadowGravity.value).toShadowGravity()
            shadowRadius = attributes.getFloat(R.styleable.CircularImageView_shadow_radius, DEFAULT_SHADOW_RADIUS)
            shadowColor = attributes.getColor(R.styleable.CircularImageView_shadow_color, shadowColor)
        }
